...
def _sanitize_tla(text: str) -> str:
    """Auto-correct the most common TLA+ syntax errors LLMs produce."""
    # 1. TLA+ uses # for not-equal, not !=
    text = text.replace("!=", "#")
    # 2. C-style // line comments → TLA+ \* comments
    text = re.sub(r"(?<!:)//", r"\\*", text)
    # 3. Footer: any run of 4+ = signs on its own line → ====
    text = re.sub(r"^={4,}\s*$", "====", text, flags=re.MULTILINE)
    # 4. Deduplicate UNCHANGED <<...>> — TLC rejects repeated names
    ...
    return re.sub(r"UNCHANGED\s*<<([^>]+)>>", _dedup, text)
